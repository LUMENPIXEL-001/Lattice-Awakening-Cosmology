...
"""
LVC final-v1.1 -- Single-file reproduction script
==================================================

Reproduces all numerical results in the final-v1.1 working paper:

    "Lagrangian Variable Cosmology, final-v1.1:
     An Exact Topological Reconstruction of a Localised
     theta-Coordinate Modulation in Late-Time Expansion History"
                                LUMENPIXEL, May 2026

This is a single self-contained Python file. No LVC-specific dependencies
beyond numpy, pandas, scipy. The likelihood backend (sections 1-11 below)
is identical to v0.1 / v0.2 / v0.3.

What this paper / script add over previous v0.x
-----------------------------------------------
v0.1-v0.3 documented the F2 free-fit chi^2 = 1600.997 vs the v0.1 locked
chi^2 = 1602.77 (with theta_c = ln(phi), w = e/(5pi)) and identified
the +1.78 deficit as localised to the width lock.

final-v1.1 introduces (i) an exact topological reconstruction of the
localised theta-coordinate modulation as the half-twist sine-Gordon kink
sector, (ii) two structural relations reducing the modulation parameter
count, and (iii) a 47x dataset scale-up validation (N=37 -> N=1738).
The script performs:
    ...
    Sec. 13.  Width relation w = A/ln phi (Sec. 5.1 of paper).
              Free-A fit on PP-excluded (N=37) with w tied to A.

    Sec. 14.  Recursive Omega_m relation (Sec. 5.2 of paper).
    ...
    Sec. 15.  Combined fit on PP-excluded data (Sec. 5.3 Table 1).

    Sec. 16.  PP-included scale-up (NEW, Sec. 3.3-3.4 of paper).
              Refit the full lock pattern on N=1738 (PP-included) and
              the four-row ladder of progressive parameter relaxation
              demonstrating Delta chi^2 = +1.00 vs the unconstrained
              six-parameter best fit.

Reproduces (key paper numbers):
-------------------------------
v0.1 sanity (chi^2 within 0.005 of v0.1 paper, PP-included):
...
Sec. 12 Lagrangian identities (analytic, no data):
    Eq. (4) sin^2(vartheta_kink/4) = sech^2(...):  residual < 1e-15
...
Sec. 15 Combined fit, PP-excluded, N=37 (paper Sec. 5.3 Table 1):
...
Sec. 16 PP-included scale-up, N=1738 (paper Sec. 3.3-3.4):
    Lambda-CDM (k=3):                  chi^2 = 1624.848
    Full lock (Om recursive, k=2):     chi^2 = 1601.999, Delta BIC = -30.31
    Same locks, Om free (k=3):         chi^2 ~ 1601.95
    theta_c lock + w=A/ln phi (k=4):   chi^2 ~ 1601.07
    All six parameters free (k=6):     chi^2 ~ 1600.997
    Locking 4 of 6 modulation params costs Delta chi^2 = +1.00.
...
Approximate runtime
-------------------
Default fast run (single seed):                                ~10 min
With --skip-baselines (no v0.1 PP-included sanity):            ~ 8 min
With --skip-pp-included (PP-excluded only):                    ~ 3 min
With --skip-baselines --skip-pp-included:                      ~ 2 min
With --multiseed (2 seeds per fit):                            ~20 min

Progress display
----------------
Each fit prints a single live status line during differential-evolution
search (iteration count, elapsed time), updated in place. In non-TTY
environments (file redirects, CI logs) the live updates are suppressed
and only step-summary lines are emitted, so log files stay clean.

Usage
-----
    python lvc_v04_reproduce.py                      # default (full run)
    python lvc_v04_reproduce.py --multiseed          # 2 seeds per fit
    python lvc_v04_reproduce.py --skip-baselines     # no v0.1 sanity
    python lvc_v04_reproduce.py --skip-pp-included   # PP-excluded only
    python lvc_v04_reproduce.py --save-json PATH     # save numbers
    python lvc_v04_reproduce.py --help               # full options

License
-------
MIT License. Copyright (c) 2026 LUMENPIXEL. See LICENSE file.
"""
...
# ============================================================================
# SECTION 8.  v0.1 SANITY-CHECK FITTERS  (with live progress display)
# ============================================================================

# TTY detection for progress display: in non-terminal environments
# (file redirects, CI logs) carriage-return updates would flood the log,
# so progress lines are suppressed there.
_IS_TTY = sys.stdout.isatty()


def _progress_line(msg):
    """Emit an in-place progress update if running in a terminal.

    Uses carriage return + flush. In non-TTY environments (redirected
    output, CI logs) this is a no-op so logs stay clean.
    """
    if _IS_TTY:
...
def _progress_clear():
    """Finish a progress line: write newline if TTY, else nothing."""
    if _IS_TTY:
        sys.stdout.write("\n")
        sys.stdout.flush()
...
def _de_nm(cost, bounds, seeds, maxiter=120, popsize=12, polish_iter=2500,
           progress_label=None):
    """Differential evolution + Nelder-Mead polish with live progress.

    A single-line in-place status display ('seed=X iter=Y/maxiter elapsed=Zs')
    is updated during the DE phase. NM polish is fast and is reported as a
    short summary. In non-TTY environments the live updates are suppressed
    automatically.
    """
    best = (1e10, None)
    label = progress_label or "fit"
    for seed_idx, sd in enumerate(seeds, start=1):
        seed_tag = (f"  [seed {seed_idx}/{len(seeds)}]"
                    if len(seeds) > 1 else "")
        t_start = time.time()
        iter_count = [0]
        ...
        def callback(xk, convergence):
            ...
            _progress_line(
                f"      {label}{seed_tag}: "
                f"DE iter {iter_count[0]:3d}/{maxiter}  "
                f"elapsed={elapsed:5.1f}s"
            )
            return False  # do not stop early

        de = differential_evolution(cost, bounds, seed=sd, tol=1e-7,
                                    maxiter=maxiter, polish=False,
                                    popsize=popsize, callback=callback)
        # Finish DE progress line, then run Nelder-Mead polish
        _progress_clear()
        _progress_line(
            f"      {label}{seed_tag}: NM polish ..."
        )
...
def fit_LCDM(pp, seeds, progress_label="LCDM"):
    bounds = [(60, 80), (0.20, 0.45), (0.020, 0.025)]
    def cost(p):
        return chi2_full(p[0], p[1], p[2], None, pp)
    return _de_nm(cost, bounds, seeds=seeds, progress_label=progress_label)


def fit_LockPhiA(pp, seeds, progress_label="LockPhi-A"):
...
def fit_LVC(pp, seeds, progress_label="LVC pendulum"):
...
# ============================================================================
# SECTION 9.  LOCKED-FORM SWEEP  (theta_c = ln phi, w = candidate)
# ============================================================================

def fit_F2_locked(pp, theta_c, w, seeds, no_pp=False, progress_label=None):
    """k=4 locked F2 fit. (H0, Om, Ob, A) free; theta_c, w fixed.

    Uses the same live progress-display pattern as _de_nm.
    """
    bounds = [(60, 80), (0.20, 0.45), (0.020, 0.025), (-0.3, 0.3)]
    def cost(p):
        H0, Om, Ob, A = p
    ...
    best = (1e10, None)
    label = progress_label or "F2 locked"
    maxiter = 80
    for seed_idx, sd in enumerate(seeds, start=1):
        seed_tag = (f"  [seed {seed_idx}/{len(seeds)}]"
                    if len(seeds) > 1 else "")
        ...
        de = differential_evolution(cost, bounds, seed=sd, tol=1e-7,
                                    maxiter=maxiter, popsize=12, polish=False,
                                    mutation=(0.5, 1.5), recombination=0.7,
                                    callback=callback)
        _progress_clear()
        _progress_line(f"      {label}{seed_tag}: NM polish ...")
        nm = minimize(cost, de.x, method='Nelder-Mead',
...
# ============================================================================
# SECTION 10.  1-D PROFILE LIKELIHOOD IN W
# ============================================================================
...
# ============================================================================
# SECTION 11.  RECONSTRUCTION-SIDE CONSTANTS (final-v1.1)
# ============================================================================
...
def T_sech2(z, A, theta_c=THETA_C_PHI, w=LNPHI_3):
    """Reconstructed sech^2 modulation in theta-coordinate (from half-twist kink)."""
    theta = np.log(1.0 + z)
    return 1.0 + A * (1.0 / np.cosh((theta - theta_c) / w))**2


# ============================================================================
# SECTION 12.  LAGRANGIAN-SIDE IDENTITIES (Sec. 4 of paper)
# ============================================================================

def verify_lagrangian_identities():
    """
    Verify Eqs. (4), (6), (7) of the paper to machine precision.
...
# ============================================================================
# SECTION 13.  WIDTH RELATION w = A / ln phi  (Sec. 5.1 of paper)
# ============================================================================

def test_width_relation(seeds):
    """
    Sec. 5.1 of paper: free A with w tied to A by w = A/ln phi.
    ...
    chi2, x = _de_nm(cost,
               ...
               progress_label="width relation w=A/ln phi")
    H0, Om, Ob_h2, A_fit = x
...
# ============================================================================
# SECTION 14.  RECURSIVE Omega_m  (Sec. 5.2 of paper)
# ============================================================================

def test_recursive_Om(seeds):
    """
    Sec. 5.2 of paper: lock Omega_m = 1 / (1 + exp(3 ln phi / phi)).
    ...
    chi2_free, x_free = _de_nm(cost_free,
                ...
                seeds=seeds, maxiter=120,
                progress_label="Om FREE reference")
    ...
    chi2_rec, x_rec = _de_nm(cost_rec,
                   ...
                   seeds=seeds, maxiter=120,
                   progress_label="Om recursive lock")
    ...
    chi2_2pp, x_2pp = _de_nm(cost_2pp,
                   [(60.0, 80.0), (0.018, 0.028)],
                   seeds=seeds, maxiter=120,
                   progress_label="Om = 1/(2+phi) [falsified]")
...
# ============================================================================
# SECTION 15.  COMBINED FIT TABLE 1  (Sec. 5.3 of paper)
# ============================================================================

def make_table_1(seeds):
    """
    Sec. 5.3 Table 1 of paper: combined fit comparison (PP-excluded, N=37).
    ...
    chi2_lcdm, _ = _de_nm(cost_lcdm,
                    [(60.0, 80.0), (0.20, 0.40), (0.018, 0.028)],
                    seeds=seeds, maxiter=120,
                    progress_label="LCDM (PP-excluded)")
    ...
    chi2_int, _ = _de_nm(cost_int,
                   [(60.0, 80.0), (0.20, 0.40), (0.018, 0.028)],
                   seeds=seeds, maxiter=120,
                   progress_label="intermediate (Om free)")
    ...
    chi2_full_v, _ = _de_nm(cost_full,
                    [(60.0, 80.0), (0.018, 0.028)],
                    seeds=seeds, maxiter=120,
                    progress_label="full lock (Om recursive)")
...
# ============================================================================
# SECTION 16.  PP-INCLUDED SCALE-UP  (Sec. 3.3-3.4 of paper, NEW)
# ============================================================================

def make_table_pp_included(pp, seeds):
    """
    Sec. 3.3-3.4 of the paper: 47x scale-up from N=37 to N=1738.

    Four rows:
      [1/4] Lambda-CDM (k=3)               -- reference baseline
      [2/4] Full lock (k=2)                -- main result, DeltaBIC = -30.31
      [3/4] Same locks, Omega_m free (k=3)
      [4/4] theta_c lock + A free + w=A/lnphi (k=4)

    The k=6 fully-free reference is taken from v0.3 paper (chi^2 = 1600.997)
    and not refitted here (six-parameter DE on N=1738 is slow and the v0.3
    value is reproducible from the v0.3 reproduction script).
    """
    print("=" * 76)
    print("Sec. 16 PP-INCLUDED SCALE-UP (Sec. 3.3-3.4 of paper)")
    print("       N=1738 (Pantheon+ unbinned + DESI + BOSS + eBOSS + DP + SH0ES)")
    print("=" * 76)

    N = 1738

    # Row 1: Lambda-CDM (reference for Delta BIC)
    def cost_lcdm(p):
        H0, Om, Ob_h2 = p
        c = chi2_full(H0, Om, Ob_h2, None, pp)
        return c if np.isfinite(c) else 1e10
    ...
    chi2_lcdm, _ = _de_nm(cost_lcdm,
                    [(60.0, 80.0), (0.20, 0.40), (0.018, 0.028)],
                    seeds=seeds, maxiter=120,
                    progress_label="LCDM (PP-included)")
    bic_lcdm = chi2_lcdm + 3 * np.log(N)
    print(f"      chi^2 = {chi2_lcdm:.4f}, BIC = {bic_lcdm:.3f}  "
          f"({time.time()-t0:.1f}s)")
    print(f"      (paper reference: chi^2 = 1624.848)")

    # Row 2: Full lock (k=2): Omega_m recursive, all (ln phi)^k modulation locks
    def cost_full(p):
        H0, Ob_h2 = p
        T = lambda z: T_sech2(z, A=LNPHI_4, theta_c=THETA_C_PHI, w=LNPHI_3)
        c = chi2_full(H0, OM_RECURSIVE, Ob_h2, T, pp)
        return c if np.isfinite(c) else 1e10

    print("\n[2/4] Full lock (Omega_m recursive, all (ln phi)^k locks; k=2) ...")
    t0 = time.time()
    chi2_full_v, x_full = _de_nm(cost_full,
                    [(60.0, 80.0), (0.018, 0.028)],
                    seeds=seeds, maxiter=120,
                    progress_label="full lock (PP-included)")
    bic_full = chi2_full_v + 2 * np.log(N)
    delta_full = bic_full - bic_lcdm
    delta_chi2_full = chi2_full_v - chi2_lcdm
    print(f"      chi^2 = {chi2_full_v:.4f}, BIC = {bic_full:.3f}, "
          f"DeltaBIC = {delta_full:+.3f}  ({time.time()-t0:.1f}s)")
    print(f"      Delta chi^2 vs LCDM: {delta_chi2_full:+.3f}")
    print(f"      (paper reference: chi^2 = 1601.999, DeltaBIC = -30.31)")

    # Row 3: Same modulation locks but Omega_m free
    def cost_int(p):
        H0, Om, Ob_h2 = p
        T = lambda z: T_sech2(z, A=LNPHI_4, theta_c=THETA_C_PHI, w=LNPHI_3)
        c = chi2_full(H0, Om, Ob_h2, T, pp)
        return c if np.isfinite(c) else 1e10
    ...
    chi2_int, _ = _de_nm(cost_int,
                   [(60.0, 80.0), (0.20, 0.40), (0.018, 0.028)],
                   seeds=seeds, maxiter=120,
                   progress_label="Om free (PP-included)")
    bic_int = chi2_int + 3 * np.log(N)
    delta_int = bic_int - bic_lcdm
    print(f"      chi^2 = {chi2_int:.4f}, BIC = {bic_int:.3f}, "
          f"DeltaBIC = {delta_int:+.3f}  ({time.time()-t0:.1f}s)")

    # Row 4: theta_c lock + A and Omega_m free, w tied to A by w = A / ln phi
    def cost_widthrel(p):
        H0, Om, Ob_h2, A = p
        if A <= 0:
            return 1e10
        ...
        if w < 0.02 or w > 0.5:
            return 1e10
        T = lambda z: T_sech2(z, A=A, theta_c=THETA_C_PHI, w=w)
        c = chi2_full(H0, Om, Ob_h2, T, pp)
        return c if np.isfinite(c) else 1e10

    print("\n[4/4] theta_c lock + A,Om free, w=A/ln phi tied (k=4) ...")
    t0 = time.time()
    chi2_widthrel, x_wr = _de_nm(cost_widthrel,
                    [(60.0, 80.0), (0.20, 0.40), (0.018, 0.028), (0.005, 0.20)],
                    seeds=seeds, maxiter=120,
                    progress_label="width relation (PP-included)")
    bic_widthrel = chi2_widthrel + 4 * np.log(N)
    delta_widthrel = bic_widthrel - bic_lcdm
    print(f"      chi^2 = {chi2_widthrel:.4f}, BIC = {bic_widthrel:.3f}, "
          f"DeltaBIC = {delta_widthrel:+.3f}  ({time.time()-t0:.1f}s)")

    # k=6 reference taken from v0.3 (not refitted)
    chi2_k6_ref = 1600.997  # from v0.3 paper, F2 free fit on N=1738

    # Summary: Delta chi^2 ladder
    print("\n" + "-" * 76)
    print("LADDER: parameter relaxation cost (PP-included, N=1738)")
    print("-" * 76)
    print(f"  {'Configuration':<48s} {'k':>3s} {'chi^2':>10s} {'Dchi^2':>9s}")
    print("-" * 76)
    print(f"  {'Full lock (Eq. 4)':<48s} {2:>3d} "
          f"{chi2_full_v:>10.3f} {chi2_full_v - chi2_k6_ref:>+9.3f}")
    print(f"  {'Locks on theta_c, A, w; Omega_m free':<48s} {3:>3d} "
          f"{chi2_int:>10.3f} {chi2_int - chi2_k6_ref:>+9.3f}")
    print(f"  {'theta_c lock; A, Om free; w=A/ln phi':<48s} {4:>3d} "
          f"{chi2_widthrel:>10.3f} {chi2_widthrel - chi2_k6_ref:>+9.3f}")
    print(f"  {'All six parameters free (v0.3 ref)':<48s} {6:>3d} "
          f"{chi2_k6_ref:>10.3f} {0.0:>+9.3f}")
    print("-" * 76)
    print(f"  Locking 4 of 6 modulation parameters costs "
          f"Delta chi^2 = {chi2_full_v - chi2_k6_ref:+.3f}")
    print(f"  relative to the unconstrained six-parameter best fit.")
    print("-" * 76)
    print()

    return {
        'lcdm_pp_inc':       {'chi2': float(chi2_lcdm),     'bic': float(bic_lcdm),
                              'k': 3},
        'full_lock_pp_inc':  {'chi2': float(chi2_full_v),   'bic': float(bic_full),
                              'k': 2, 'delta_bic': float(delta_full),
                              'delta_chi2_vs_lcdm': float(delta_chi2_full)},
        'int_pp_inc':        {'chi2': float(chi2_int),      'bic': float(bic_int),
                              'k': 3, 'delta_bic': float(delta_int)},
        'widthrel_pp_inc':   {'chi2': float(chi2_widthrel), 'bic': float(bic_widthrel),
                              'k': 4, 'delta_bic': float(delta_widthrel)},
        'k6_v03_ref':        {'chi2': float(chi2_k6_ref),   'k': 6,
                              'note': 'taken from v0.3 paper, not refitted here'},
        'lock_cost_vs_k6':   float(chi2_full_v - chi2_k6_ref),
    }


# ============================================================================
# SECTION 17.  v0.1 BASELINE SANITY CHECK (PP-included, optional)
# ============================================================================

def run_baselines(pp, seeds):
    """Reproduce v0.1 paper headline numbers."""
    print("=" * 76)
    print("Sec. 17 BASELINES: reproduce v0.1 headline numbers (PP-included, N=1738)")
    print("=" * 76)
...
def _step_header(step_num, n_steps, label, est_min):
    """Print a clearly visible step header with progress info."""
    print()
    print("#" * 76)
    print(f"#  Step {step_num}/{n_steps}: {label}")
    print(f"#  (estimated ~{est_min} on a single core)")
    print("#" * 76)


def main():
    parser = argparse.ArgumentParser(
        description="Reproduce LVC final-v1.1 working paper numerical results.",
        formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument('--multiseed', action='store_true',
                        help="use 2 seeds per fit (slower, more robust)")
    parser.add_argument('--skip-baselines', action='store_true',
                        help="skip v0.1 PP-included sanity check (Sec. 17)")
    parser.add_argument('--skip-pp-included', action='store_true',
                        help="skip PP-included scale-up (Sec. 16); "
                             "PP-excluded analysis only")
    ...
    # Plan the steps so we can show "Step X/Y" headers.
    # Step 1 (identities) is always run; data loading is its own line; the
    # remaining steps depend on the skip flags.
    steps = [('Lagrangian identities (analytic, no data)', '< 1s')]
    if not args.skip_pp_included:
        steps.append(('PP-included scale-up (Sec. 3.3-3.4 of paper, N=1738)',
                      '5 min' if not args.multiseed else '10 min'))
    if not args.skip_baselines:
        steps.append(('v0.1 baselines (PP-included sanity, N=1738)',
                      '2 min' if not args.multiseed else '4 min'))
    steps.append(('Width relation (Sec. 5.1, PP-excluded N=37)',
                  '30s' if not args.multiseed else '1 min'))
    steps.append(('Recursive Omega_m (Sec. 5.2, PP-excluded N=37)',
                  '1 min' if not args.multiseed else '2 min'))
    steps.append(('Combined fit Table 1 (Sec. 5.3, PP-excluded N=37)',
                  '1.5 min' if not args.multiseed else '3 min'))
    n_steps = len(steps)

    print("=" * 76)
    print("LVC final-v1.1 -- Standalone reproduction script")
    print("=" * 76)
    print(f"  phi          = {PHI:.10f}")
    ...
    print(f"  TTY detected: {_IS_TTY}  "
          f"(progress lines {'enabled' if _IS_TTY else 'suppressed in non-TTY'})")
    print()
    ...
    # Step 1: Sec. 12 -- Lagrangian identities (no data)
    step += 1
    ...
    # Step 2: Sec. 16 -- PP-included scale-up (NEW, headline ΔBIC = -30.31)
    if not args.skip_pp_included:
        step += 1
        _step_header(step, n_steps, steps[step-1][0], steps[step-1][1])
        results['pp_included_scaleup'] = make_table_pp_included(pp, seeds)
    else:
        print("Skipping PP-included scale-up (--skip-pp-included).\n")

    # Step 3: Sec. 17 -- v0.1 baselines (optional sanity check)
    if not args.skip_baselines:
        step += 1
        _step_header(step, n_steps, steps[step-1][0], steps[step-1][1])
    ...
    # Step 4: Sec. 13 -- width relation (PP-excluded)
    step += 1
    _step_header(step, n_steps, steps[step-1][0], steps[step-1][1])
    results['width_relation'] = test_width_relation(seeds)

    # Step 5: Sec. 14 -- recursive Omega_m (PP-excluded)
    step += 1
    _step_header(step, n_steps, steps[step-1][0], steps[step-1][1])
    results['recursive_Om'] = test_recursive_Om(seeds)

    # Step 6: Sec. 15 -- Table 1 (PP-excluded)
    step += 1
    _step_header(step, n_steps, steps[step-1][0], steps[step-1][1])
    ...
    if 'pp_included_scaleup' in results:
        s = results['pp_included_scaleup']
        print()
        print("Sec. 16 PP-included scale-up (Sec. 3.3-3.4 of paper, N=1738):")
        print(f"  Lambda-CDM (k=3):           chi^2 = {s['lcdm_pp_inc']['chi2']:.3f}     "
              f"(ref 1624.848)")
        print(f"  Full lock (k=2):            chi^2 = {s['full_lock_pp_inc']['chi2']:.3f}     "
              f"(ref 1601.999)")
        print(f"  DeltaBIC vs LCDM:           {s['full_lock_pp_inc']['delta_bic']:+.3f}     "
              f"(ref -30.31)")
        print(f"  Same locks, Om free (k=3):  chi^2 = {s['int_pp_inc']['chi2']:.3f}")
        print(f"  Width relation tied (k=4):  chi^2 = {s['widthrel_pp_inc']['chi2']:.3f}")
        print(f"  k=6 free (v0.3 ref):        chi^2 = {s['k6_v03_ref']['chi2']:.3f}")
        print(f"  Cost of locking 4 of 6:     Dchi^2 = {s['lock_cost_vs_k6']:+.3f}     "
              f"(ref +1.00)")

    if 'baselines' in results:
        b = results['baselines']
        print()
        print("Sec. 17 v0.1 baselines (PP-included, N=1738):")
    ...
    print(f"  Full lock (Omega_m recursive, k=2):    "
          f"DeltaBIC = {t1['full_lock']['delta_bic']:+.3f}     (ref -26.44)")
